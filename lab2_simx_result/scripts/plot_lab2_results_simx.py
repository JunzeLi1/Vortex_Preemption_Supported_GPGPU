...
LOG_DIR = SCRIPT_DIR.parent          # /home/andyl/vortex/lab2_simx_result
OUT_DIR = LOG_DIR                    # save PNGs next to logs

FORMATS = ["fp16", "bf16", "tf32"]   # we care about these three
...
def parse_logs():
    """
    Parse all *.log files under LOG_DIR.
        ...
        with log_path.open() as f:
            ...
            for raw in f:
                ...
                if line.startswith("CONFIGS="):
                    ...
                    if current is not None and {"fmt", "warps", "threads", "instrs", "cycles"} <= current.keys():
                        records.append(current)
                    current = {}

                    # Example:
                    # CONFIGS=-DEXT_TCU_ENABLE -DNUM_CORES=4 -DNUM_WARPS=4 -DNUM_THREADS=4 -DPERF_ENABLE
                    tokens = line.split()
                    for tok in tokens:
                ...
                # Aggregate instrs/cycles line
                elif line.startswith("PERF: instrs=") and current is not None:
                    # line: PERF: instrs=455128, cycles=161679, IPC=2.815010
                    ...
                    # we assume one PERF summary per CONFIGS block
                    records.append(current)
                    current = None
...
def make_plots(records):
    ...
    # Find configs where we have all three formats
    ...
    if not valid_configs:
        print("[ERROR] No config has all of fp16/bf16/tf32. Check logs.")
        return

    print("Using configs (warps, threads):", valid_configs)

    # Print a small table for copy-paste into the report
    print("\n=== Summary Table ===")
    ...
    plt.title("sgemm_tcu @ SimX: Instruction Count vs. Format")
    plt.legend()
    plt.tight_layout()
    instrs_png = OUT_DIR / "lab2_instrs.png"
    ...
    plt.title("sgemm_tcu @ SimX: Cycle Count vs. Format")
    plt.legend()
    plt.tight_layout()
    cycles_png = OUT_DIR / "lab2_cycles.png"
